...
# pegasus-planner will, by default, pick up this file in cwd
sc.write()
LOGGER.info("Creating sites catalog")
...
# pegasus-planner will, by default, pick up this file in cwd
tc.write()
LOGGER.info("Creating transformations catalog")
...
rc = ReplicaCatalog() \
        ...
        .add_replica("local", input_dev, str(Path(__file__).parent.resolve() / "input/para/" / input_dev.lfn))
...
wf = Workflow(WF_ID, infer_dependencies=True)
...
for lang in LANGS:
    ...
    for year in YEARS:
        ...
        if current_input.lfn not in files_already_there:
            LOGGER.info("{} will be downloaded from {}..".format(current_input, BASE_URL))
            ...
            wf.add_jobs(wget[lang][year])
        else:
            LOGGER.info("{0} found in {1}".format(current_input, MONO_PATH))
        ...
        if dataset[lang][year].lfn not in files_already_there:
            ...
            LOGGER.info("{0} already unzipped in {1}".format(current_input, MONO_PATH))
    ...
    wf.add_jobs(tokenize[lang])
    LOGGER.info("{0} monolingual data tokenized in: {1}".format(lang, lang_tok[lang]))
...
for lang in LANGS:
    ...
    wf.add_jobs(apply_bpe[lang])
    LOGGER.info("BPE codes applied to {0} in: {1}".format(lang, tok_codes[lang]))
    ...
    wf.add_jobs(extract_vocab[lang])
    LOGGER.info("{0} vocab in: {1}".format(lang, lang_vocab[lang]))
...
for lang in LANGS:
    ...
    wf.add_jobs(binarize[lang])

    LOGGER.info("{0} binarized data in: {1}".format(lang, lang_binarized[lang]))
...
for lang in LANGS:
    ...
    job_valid[lang].add_inputs(input_dev) # the dev.tgz containing data for validations
    ...
    wf.add_jobs(job_valid[lang])

    LOGGER.info("Tokenizing valid {0} data {1}".format(lang, file_valid[lang]))
    ...
    job_test[lang].add_inputs(input_dev)
    ...
    wf.add_jobs(job_test[lang])

    LOGGER.info("Tokenizing test {0} data {1}".format(lang, file_test[lang]))
...
for lang in LANGS:
    ...
    wf.add_jobs(job_apply_valid[lang])
    LOGGER.info("BPE codes for validation applied to {0} in: {1}".format(lang, file_apply_valid[lang]))
    ...
    wf.add_jobs(job_apply_test[lang])
    LOGGER.info("BPE codes for test applied to {0} in: {1}".format(lang, file_apply_test[lang]))
...
for lang in LANGS:
    ...
    wf.add_jobs(job_binarize_valid[lang])
    LOGGER.info("{0} binarized valid data in: {1}".format(lang, file_binarize_valid[lang]))
    ...
    wf.add_jobs(job_binarize_test[lang])
    LOGGER.info("{0} binarized test data in: {1}".format(lang, file_binarize_test[lang]))
...
wf.add_jobs(fasttext)
LOGGER.info("Cross-lingual embeddings (pre-training results) in: {0}".format(bpe_vec))
...
wf.add_jobs(training)
LOGGER.info("Model trained => {0}".format(training_out))

###########################################################################
#############################  End Training ###############################
###########################################################################

LOGGER.info("Creating workflow done.")
